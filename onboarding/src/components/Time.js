...
class Time extends React.Component {
  ...
  handleNext = e => {
    e.preventDefault();
    const time = Object.keys(this.state.selectedTimes)
      .filter(day => this.state.selectedTimes[day].length > 0)
      .map(day => ({
        day: day,
  ...
  render() {
    ...
    return (
      <section className="time">
        ...
        <p>
          To complete the beta, we are asking that people commit to one 30
          minute time block once a week. Please choose what times work well for
          you.
        </p>
        ...
        <div className="days">
          <Slider {...settings}>
            {Object.keys(this.state.selectedTimes).map(day => (
              <Day day={day} key={day} selectTime={this.selectTime} />
            ))}
          </Slider>
        </div>
